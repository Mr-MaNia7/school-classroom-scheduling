...
class genetical_sort2():
    def __init__(self, class_instances, subject_instances, teacher_instances, teach_instances):
        ...
        self.subject_dict_num = dict(zip(range(len(subject_instances)), [
                                     subj.subject_name for subj in subject_instances]))
        # Dict type to quicken subject object access
        self.subject_dict_name = dict([i.subject_name, i]
                                      for i in self.subject_instance)

        self.teacher_dict_num = dict(zip(range(len(teacher_instances)), [
                                     x.teacher_name for x in teacher_instances]))
        # Dict type to quicken teacher object access
        self.teacher_dict_name = dict([i.teacher_name, i]
    ...
    def initialize_teacher_individual(self):
        ...
        for classs in self.class_instance:
            ...
            classs_copy = list(filter(lambda element: element !=
                               "Recess period" and element != "Assembly", classs_copy))

            for subj in classs.subjects.keys():

                if self.subject_dict_name[subj].specialty == True:
                    temp_teach = random.choice([key for key, val in self.teacher_dict_num.items(
                    ) if self.teacher_dict_name[val].specialty_subject.subject_name == subj])
                else:
                    temp_teach = random.choice(
                        list(self.teacher_dict_num.keys()))
    ...
    def initialize_period_address(self):
        ...
        # Number of class section for each class. 2 periods of the same subjects joined together is considered as a section.
        Section = []
        ...
        for classs in self.class_instance:
            ...
            for sub in list(classs.subjects.keys()):
                ...
                subject_object = next(
                    i for i in self.subject_instance if i.subject_name == sub)
                ...
                if subject_object.side_by_side == True:
                    ...
                    while period_remaining > 0 and (period_remaining/2) >= 1:
                        ...
                        period_address_dictionary[period_address_counter] = [
                            sub, 2]
                        period_remaining -= 2
    ...
    def initialize_schedule_individual(self):
        ...
        for num, i in enumerate(self.section_per_class):
            ...
            temp_list = list(range(sum(self.section_per_class[:num]), sum(
                self.section_per_class[:num]) + i))
    ...
    def evaluate_schedule_assignment(self, individual):
        ...
        # Dict type to quicken subject object access
        subject_dict_name = dict([i.subject_name, i] for i in subject_instance)
        teacher_dict_name = dict([i.teacher_name, i] for i in teacher_instance)
        ...
        for classs in class_instance:
            ...
            for day_num, day in enumerate(classs.schedule):
                ...
                for period_num, period in enumerate(day):
                    ...
                    if period != "Recess period" and period != "Assembly":
                        ...
                        if Subject_chosen_number == 0:
                            ...
                            Subject_chosen_number = self.period_address[
                                decoded_individual[individual_counter]][1]
                ...
                my_dict = Counter(
                    i.subject for i in classs.schedule[day_num] if not isinstance(i, str))
                ...
                for teach_activity, total_period in my_dict.items():
                    ...
                    Subject_object = next(
                        i for i in subject_instance if i.subject_name == teach_activity)
            ...
            fitness -= sum(filter(lambda k: k >= 0,
                           map(lambda x: abs(x[0]), list(classs.subjects.values()))))
        ...
        for num, clas in enumerate(period_number_list):
            for i in range(1):
                if random.random() < indpb:
                    ...
                    individual[period_choice[0]:period_choice[1] +
                               1] = reversed(individual[period_choice[0]:period_choice[1]+1])
    ...
    def heuristic_mutation(self, individual, period_number_list, gamma=3):
        ...
        for num, clas in enumerate(period_number_list):
            ...
            Selected_index = random.sample(range(
                sum(period_number_list[:num]), sum(period_number_list[:num]) + clas), gamma)
            ...
            for nummer, permut in enumerate(Permutation_suggestion):
                ...
                Score.append(self.evaluate_schedule_assignment(
                    temp_individual)[0])

            # Select permutation with highest fitness score
            for number, i in enumerate(Selected_index):
                individual[i] = Permutation_suggestion[Score.index(
                    max(Score))][number]
    ...
    def assign_teacher(self, solution):
        ...
        for classs in self.class_instance:
            for subject in list(classs.subjects.keys()):
                ...
                classs.subjects[subject].append(
                    self.teacher_dict_num[solution[counter]])

                teacher = next(
                    teacher for teacher in self.teacher_instance if self.teacher_dict_num[solution[counter]] == teacher.teacher_name)
    ...
    def print_teacher_assigned(self, teacher):
        ...
        for classs in self.class_instance:
            ...
            for sub_name in list(classs.subjects.keys()):
                print(sub_name, " : ",
                      self.teacher_dict_num[teacher[counter]], end="   ")
                counter += 1
    ...
    def main(self):
        ...
        toolbox.register("individual", tools.initIterate,
                         creator.Individual, self.initialize_teacher_individual)
        toolbox.register("population", tools.initRepeat,
                         list, toolbox.individual, n=300)
        toolbox.register("populations", tools.initRepeat,
                         list, toolbox.population)

        toolbox.register("evaluate", self.evaluate_teacher_assignment)
        toolbox.register("mate", tools.cxUniform, indpb=0.3)
        toolbox.register("mutate", self.ID_mutation, Mutation_material=list(
            range(len(self.teacher_instance))), indpb=0.1)
        ...
        solution = next(
            indiv for indiv in pop if self.evaluate_teacher_assignment(indiv)[0] == 0)
        ...
        toolbox.register("individual", tools.initIterate,
                         creator.Individual, self.initialize_schedule_individual)
        ...
        toolbox.register("mate", tools.cxOrdered_by_class,
                         period_number_list=self.section_per_class)
        ...
        toolbox.register("mutate", self.heuristic_mutation,
                         gamma=3, period_number_list=self.section_per_class)
